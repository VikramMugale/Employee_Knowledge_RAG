"""Session JWT issuance after a verified OAuth/OIDC login. Local password accounts were removed."""
...
class AuthenticationService:
    ...
    def authenticate_token(self, token: str) -> UserContext:
        ...
        if raw.lower().startswith("bearer "):
            raw = raw[7:].strip()
        try:
            payload = jwt.decode(raw, settings.jwt_secret, algorithms=[settings.jwt_algorithm], audience=settings.jwt_audience, issuer=settings.jwt_issuer)
    ...
    def user_from_context(self, context: UserContext) -> User:
        return User(id=context.user_id, email=context.email or f"{context.user_id}@unknown", full_name=context.full_name or context.user_id, role=context.role, department=context.department, location=context.location, employment_type=context.employment_type)
